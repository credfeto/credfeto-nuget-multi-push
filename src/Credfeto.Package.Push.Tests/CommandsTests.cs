...
public sealed class CommandsTests : LoggingFolderCleanupTestBase
{
    ...
    private const string ENVIRONMENT_API_KEY = "env-test-key";
    ...
    [Fact]
    public async Task UploadPackagesWithNoApiKeyFlagAndNoEnvironmentVariableThrowsUploadConfigurationErrorsExceptionAsync()
    {
        string packagePath = Path.Combine(path1: this.TempFolder, path2: "MyLib.1.0.0.nupkg");
        await File.WriteAllTextAsync(
            path: packagePath,
        ...
        Environment.SetEnvironmentVariable(variable: EnvironmentVariables.NugetApiKey, value: null);

        Commands commands = new(this._uploadOrchestration);

        await Assert.ThrowsAsync<UploadConfigurationErrorsException>(() =>
            commands.UploadPackagesAsync(source: SOURCE, folder: this.TempFolder, apiKey: null, symbolSource: null)
        );
    }

    [Fact]
    public async Task UploadPackagesWithNoApiKeyFlagAndEnvironmentVariableSetUsesEnvironmentValueAsync()
    {
        string packagePath = Path.Combine(path1: this.TempFolder, path2: "MyLib.1.0.0.nupkg");
        await File.WriteAllTextAsync(
            path: packagePath,
        ...
        Environment.SetEnvironmentVariable(variable: EnvironmentVariables.NugetApiKey, value: ENVIRONMENT_API_KEY);
        ...
        await this
            ._uploadOrchestration.Received(1)
            .PushAllAsync(
                source: SOURCE,
                symbolSource: null,
                packages: Arg.Any<IReadOnlyList<string>>(),
    ...
    [Fact]
    public async Task UploadPackagesWithApiKeyFlagAndEnvironmentVariableSetPrefersFlagValueAsync()
    {
        string packagePath = Path.Combine(path1: this.TempFolder, path2: "MyLib.1.0.0.nupkg");
        await File.WriteAllTextAsync(
            path: packagePath,
